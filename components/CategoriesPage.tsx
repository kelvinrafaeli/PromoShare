  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-500">Painel exclusivo do Administrador para padronização do sistema</p>
      </div>
          ...
          <div className="flex gap-2 p-1 bg-slate-50 border border-slate-200 rounded-xl overflow-x-auto max-w-[300px]">
            {colors.map(c => (
              <button 
                ...
                className={`w-8 h-8 rounded-lg shrink-0 transition-transform ${c} ${newCat.color === c ? 'scale-110 ring-2 ring-white ring-offset-2 ring-offset-slate-100 shadow-lg' : 'opacity-60 hover:opacity-100'}`}
              />
            ))}
          </div>
        </div>
        ...
        {state.categories.map(cat => (
          <div key={cat.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between group">
            ...
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button onClick={() => handleDelete(cat.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg">
                <Trash2 size={18} />
              </button>
            </div>
          </div>
